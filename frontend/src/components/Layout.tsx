...
const Layout: React.FC = () => {
  ...
  const getNavigationByRole = (role: string): NavItem[] => {
    ...
    const dashboardHrefMap: Record<string, string> = {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Sidebar */}
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-secondary text-white shadow-xl transform transition-transform duration-300 ease-in-out lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full">
          {/* Logo */}
          <div className="flex items-center justify-between h-16 px-5 border-b border-secondary/20 bg-secondary">
            <div className="flex items-center gap-2">
                <img src="/images/hospital.jpg.webp" alt="Alem Ketema Enat Hospital" className="w-10 h-10 rounded-lg object-cover" />
            ...
            {navigation.map((item) => {
              ...
              return (
                <Link
                  key={item.name}
                  to={item.href}
                  className={`flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-all ${isActive ? 'bg-card text-foreground border-l-4 border-primary rounded-r-lg shadow-sm' : 'text-white hover:bg-secondary/90 hover:text-white'}`}
                  onClick={() => setSidebarOpen(false)}
                >
                  <item.icon className={`w-4 h-4 mr-3 flex-shrink-0 ${isActive ? 'text-primary' : 'text-white'}`} />
                  {item.name}
                </Link>
              );
            })}
          ...
          {/* User info & logout */}
          <div className="p-4 border-t border-[#0f2a3c] bg-[#071722]">
